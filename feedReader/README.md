...
1. are url 执行循环allFeeds并判断每个list下的url属性是否存在，如果存在的话是否等于''

2. are content 先通过for循环对象列表在每个对象中再使用for in 循环内部的属性值是否为''

#3 The menu 测试

1. menu hidden通过获取.slide-menu的transform属性并用正则匹配X坐标的值来判断是否小于0小于0视在屏幕外

2. menu click通过触发点击.menu-icon-link来判断body是否有.menu-hidden来判断是否在屏幕内还是再屏幕外

#3 Initial Entries测试

1. 通过beforeEach触发loadFeed异步函数并回调done来判断.entry的class的length是否大于0忘记JQ怎么取length值了所以用的是原生写法

#3 New Feed Selection测试

1. 首先定义了变量titleHtml并在beforeEach中调用loadFeed来初始化数据并在回调函数中把$('.header-title').html()存储在变量titleHtml，存储后再次调用loadFeed并修改id参数来模拟点击事件并在回调函数done中判断修改后的$('.header-title').html()不等于titleHtml变量极为成功